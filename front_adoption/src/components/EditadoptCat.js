...
import { editadoptCat } from '../redux/adoptCatSlice';
import { deletecat } from '../redux/catSlice';

function EditadoptCat({ adoptCat }) {
  ...
  const handleEdit = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(editadoptCat({ id: adoptCat?._id, edited: { status: 'accepted' } }))
          .then(() => {
            dispatch(deletecat(adoptCat?.cat_adopted)); // Supprimer le chat une fois que la demande est acceptée
